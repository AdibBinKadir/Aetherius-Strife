 ...
 * 
 * Compile: gcc battlesimulator.c -o advanced_battle_simulator
 * Windows Run: .\battlesimulator.exe
 */
...
bool authenticate();
void register_user(char player_name[MAX_NAME]);
void login_user(char player_name[MAX_NAME]);
...
const char names[][MAX_NAME] = {"Adib", "Lakhsman", "Alexis", "Maleeha"};
...
int main() {
    ...
    printf("Welcome to the Aetherius Strife!\n\n");

    printf("Are you a new player? (y/n): ");
    char choice;
    scanf("%c", &choice);
    if (choice == 'y') {
        register_user(player_name);
    }else{
    ...
    init_player(&player1, player_name, (CharacterClass)class_choice);
    int rand_num = (int)rand() %4;
    char cpu_name[MAX_NAME];
    strncpy(cpu_name, names[rand_num], MAX_NAME);
    class_choice = (rand()%4);
    init_player(&player2, cpu_name, (CharacterClass)class_choice);
